...
import { addConnectorIdsToDiagram, addElementIdsToDiagram, mapToEaDiagram } from "./utils/diagramUtils";
...
export class SqliteFileReader implements IFileReader<SqliteDatabase> {
  public async initDataRegistry(path: string, registry: DataRegistry): Promise<DataRegistry> {
  ...
  public async loadPackages(database: SqliteDatabase, registry: DataRegistry): Promise<void> {
    ...
    const objectTags = database.prepare(`SELECT * FROM ${EaTable.ClassAndPackageTag}`).all();
    addEaTagsToElements(objectTags, registry.packages, 'Object_ID', 'Value');
  }

  public async loadElements(database: SqliteDatabase, registry: DataRegistry): Promise<void> {
    ...
    registry.elements = mapToEaElement(elementStatement.all(), registry.packages);
  ...
  public async loadAttributes(database: SqliteDatabase, registry: DataRegistry): Promise<void> {
    ...
    const attributeTags = database.prepare(`SELECT * FROM ${EaTable.AttributeTag}`).all();
    addEaTagsToElements(attributeTags, registry.attributes, 'ElementID', 'VALUE');
  }

  public async loadElementConnectors(database: SqliteDatabase, registry: DataRegistry): Promise<void> {
    ...
    registry.connectors = mapToElementConnector(elementConnectorStatement.all(), registry.elements);

    const connectorTags = database.prepare(`SELECT * FROM ${EaTable.ConnectorTag}`).all();
    addEaTagsToElements(connectorTags, registry.connectors, 'ElementID', 'VALUE');

    const roleTags = database.prepare(`SELECT * FROM ${EaTable.ConnectorRoleTag}`).all();
    addRoleTagsToElements(roleTags, registry.connectors);
  }

  public async loadDiagrams(database: SqliteDatabase, registry: DataRegistry): Promise<void> {
    const diagramStatement = database.prepare(`
      SELECT * FROM ${EaTable.DiagramObject}
    `);

    registry.diagrams = mapToEaDiagram(diagramStatement.all(), registry.packages);

    await Promise.all([
      this.loadDiagramObjects(database, registry.diagrams),
      this.loadDiagramConnectors(database, registry.diagrams, registry.connectors),
    ]);
  }

  private async loadDiagramObjects(database: SqliteDatabase, diagrams: EaDiagram[]): Promise<void> {
  ...
  private async loadDiagramConnectors(
    ...
    const diagramConnectorStatement = database.prepare(`SELECT * FROM ${EaTable.DiagramLink}`);
    addConnectorIdsToDiagram(diagramConnectorStatement.all(), diagrams, elementConnectors);
  }
}