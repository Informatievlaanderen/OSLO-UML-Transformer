...
| `--versionSPARQL`  | SPARQL version used for generating queries                   | :heavy_check_mark: |                             |
| `--silent`         | Suppress log messages                                        | No                 | `true` or `false` (default) |

## Usage
```bash
oslo-generator-sparql --input report.jsonld --output sparql.json --language nl --versionSPARQL 1.1
```
