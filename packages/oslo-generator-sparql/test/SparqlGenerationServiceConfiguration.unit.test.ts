...
describe('SparqlGenerationServiceConfiguration', () => {
  ...
  it('should set its variables using the parameters received from the CLI', async () => {
    ...
    expect((<any>config)._versionSPARQL).toBeDefined();
  });
  ...
  it('should throw an error when "versionSPARQL" is undefined or otherwise return its value', async () => {
    const config = new SparqlGenerationServiceConfiguration();

    expect(() => config.versionSPARQL).toThrowError(
      'Trying to access property "versionSPARQL" before it was set.',
    );
    await config.createFromCli(params);
    expect(config.versionSPARQL).toBe('1.1');
  });
});
