...
describe('JsonWebuniversumGenerationServiceConfiguration', () => {
	...
	beforeEach(() => {
		...
		service = new JsonWebuniversumGenerationService(
			logger,
			<JsonWebuniversumGenerationServiceConfiguration>{ language: 'en', output: 'config.json' },
			store,
		);
	});
	...
	it('should throw an error when an assigned URI is not found', async () => {
		...
		await expect(() => (<any>service).generateEntityData(df.namedNode('http://example.org/id/class/1')))
			.rejects.toThrow(new Error('Unable to find the assigned URI for entity http://example.org/id/class/1.'));
	});
	...
	it('should include properties into a WebuniversumObject if present', async () => {
		...
		const entityData = await (<any>service)
			.generateEntityData(df.namedNode('http://example.org/.well-known/id/class/1'));

		expect(entityData.properties).toHaveLength(1);
		expect(entityData.properties).toEqual(expect.arrayContaining([
	...
	it("should throw an error when the a property's range can not be found", async () => {
		...
		await expect(() => (<any>service).generateEntityData(df.namedNode('http://example.org/.well-known/id/class/1')))
			.rejects.toThrow(new Error(`No range found for class http://example.org/.well-known/id/property/1.`))
	})
	...
	it("should throw an error when the assigned URI of a property's range can not be found", async () => {
		...
		await expect(() => (<any>service).generateEntityData(df.namedNode('http://example.org/.well-known/id/class/1')))
			.rejects.toThrow(new Error(`Unable to find the assigned URI for range http://example.org/.well-known/id/class/2 of attribute http://example.org/.well-known/id/property/1.`))
	});

	it('should add parent information if it is present in the QuadStore', async () => {
		store.addQuads(await parseJsonld(classWithParent));
		const parentObject = (<any>service)
			.createParentObject(df.namedNode('http://example.org/.well-known/id/class/1'));
