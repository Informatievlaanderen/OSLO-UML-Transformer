...
describe('CodelistConstraintHandler', () => {
      ...
      shaclQuads.some(
        (quad) =>
          quad.predicate.equals(
            df.namedNode('http://purl.org/linked-data/cube#codeList'),
          ) &&
  ...
  it('should add a codelist constraints in individual mode', async () => {
    ...
    expect(
      shaclQuads.some(
        (quad) =>
          ...
          quad.object.equals(
            df.namedNode('http://www.w3.org/ns/shacl#NodeConstraint'),
          ),
      ),
    ).toBe(true);
    expect(
      shaclQuads.some(
        (quad) =>
          quad.predicate.equals(
            df.namedNode('http://www.w3.org/ns/shacl#nodeKind'),
          ) &&
    ...
    expect(
      shaclQuads.some(
        (quad) =>
          quad.predicate.equals(
            df.namedNode('http://www.w3.org/ns/shacl#minCount'),
          ) &&
          quad.object.equals(
            df.literal(
              '1',
    ...
    expect(
      shaclQuads.some(
        (quad) =>
          ...
          ) && quad.object.equals(df.namedNode(ns.skos('inScheme').value)),
      ),
    ).toBe(true);
  });
    ...
    expect(
      shaclQuads.some(
        (quad) =>
          ...
          quad.object.equals(
            df.namedNode(
              'http://example.org/id/property/shape/1-CodelistConstraint',
            ),
          ),
      ),
    ).toBe(true);
  ...
  it('should throw an error when the range can not be found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `No range found for subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
    ...
    expect(
      shaclQuads.some(
        (quad) =>
          ...
          quad.predicate.equals(
            df.namedNode('https://data.vlaanderen.be/ns/shacl#message'),
          ) &&
          quad.object.equals(
            df.literal(
              'Enkel waarden uit codelijst <http://example.org/id/codelist/1> verwacht voor "PropertyLabel".',
            ),
          ),
      ),
    ).toBe(true);
