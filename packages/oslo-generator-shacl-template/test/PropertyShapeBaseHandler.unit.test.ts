...
describe('PropertyShapeBaseHandler', () => {
  ...
  beforeEach(async () => {
    ...
    propertyIdToShapeIdMap = new Map<string, NamedOrBlankNode>([
      [
        'http://example.org/.well-known/id/property/1',
    ...
    classIdToShapeIdMap = new Map<string, NamedOrBlankNode>([
      [
        'http://example.org/.well-known/id/class/1',
        df.namedNode('http://example.org/id/class/shape/1'),
      ],
      [
        'http://example.org/.well-known/id/class/2',
        df.namedNode('http://example.org/id/class/shape/2'),
      ],
    ]);
  ...
  it('should add base quads to the quad store', async () => {
      ...
      quads.some(
        (quad: RDF.Quad) =>
          quad.predicate.equals(
            df.namedNode('http://www.w3.org/ns/shacl#path'),
    ...
    expect(
      quads.some(
        (quad: RDF.Quad) =>
          quad.predicate.equals(
            df.namedNode('http://www.w3.org/ns/shacl#name'),
    ...
    expect(
      quads.some(
        (quad: RDF.Quad) =>
          ...
          quad.object.equals(
            df.namedNode(
              'http://example.org/ap#DomainClassLabel.PropertyLabel',
            ),
          ),
      ),
    ).toBe(true);
  ...
  it('should throw an error if the assigned URI for subject is not found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find the assigned URI for subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
  ...
  it('should throw an error when the label for subject is not found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find the label for subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
  ...
  it('should throw an error when the range for subject is not found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find the range for subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
  ...
  it('should throw an error when the range assigned URI is not found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find the assigned URI for range "http://example.org/.well-known/id/class/1".`,
      ),
    );
  });
  ...
  it('should throw an error when the property type is not found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find the type for subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
  ...
  it('should throw an error when the domain label can not be found', async () => {
    ...
    expect(() =>
      ...
      new Error(
        `Unable to find a label for the domain "http://example.org/.well-known/id/class/2" of subject "http://example.org/.well-known/id/property/1".`,
      ),
    );
  });
  ...
  it('should add a vl:rule quad is the option is set in the configuration', async () => {
    ...
    expect(
      quads.some(
        (quad: RDF.Quad) =>
          quad.predicate.equals(
            df.namedNode('https://data.vlaanderen.be/ns/shacl#rule'),
          ) &&
          quad.subject.equals(
            df.namedNode('http://example.org/id/property/shape/1'),
          ) &&
